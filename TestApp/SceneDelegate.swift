...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func sceneDidDisconnect(_ scene: UIScene) {
        // Called as the scene is being released by the system.windowScene
    ...
    func createFavoriteViewController() -> UINavigationController {
        ...
        favoriteViewController.tabBarItem = UITabBarItem(title: "Favorite", image: UIImage(systemName: "heart"), tag: 0)
        return UINavigationController(rootViewController: favoriteViewController)
    }
    func createTabBarController() -> UITabBarController {
        let tabBarController = UITabBarController()
        UITabBar.appearance().backgroundColor = .secondarySystemBackground
        tabBarController.viewControllers = [createFeedViewController(), createFavoriteViewController()]
